...
class Common extends Controller
{
	...
	{
		return db('article')->where('is_recycle',2)->order('sendtime desc')->limit(2)->field('arc_id,arc_title,sendtime')->select();
	}
	/**
	 * 获取标签数据
	 */
	...
	private function loadCateData(){
		return db('cate')->where('cate_pid',0)->order('cate_sort desc')->limit(7)->select();
	}
	/**
	 * 读取配置项
	 */
